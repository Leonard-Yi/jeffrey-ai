(() => {
  ...
  var simNodes = [];
  var sim = null;
  function initSimulation(nodes, links, centerX, centerY) {
    if (sim) {
      sim.stop();
    }
    const nodeMap = new Map(nodes.map((n) => [n.id, n]));
    const resolvedLinks = links.filter(([src, tgt]) => nodeMap.has(src) && nodeMap.has(tgt)).map(([src, tgt]) => ({
  ...
  self.onmessage = (event) => {
    ...
    if (!sim && msg.nodes) {
      ...
      simNodes = Object.entries(msg.nodes).map(([id, [x3, y3]]) => ({
        id,
        x: x3,
        y: y3,
        vx: 0,
